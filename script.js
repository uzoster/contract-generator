...
(() => {
  ...
  function defaultState() {
    return {
      ...
      company: {
        ...
        address: "", phone: "", email: "", website: "", verifyBaseUrl: "", logo: "", stamp: "", signature: "",
      },
  ...
  const DOC_SLUGS = { itService: "it-service", nda: "nda", sow: "sow", acceptance: "acceptance", invoice: "invoice", maintenance: "maintenance" };

  function signLabelsFor() {
    return {
      companyLabel: t("field.companyName"), tinLabel: t("field.tin"), addressLabel: t("field.address"),
      bankLabel: t("field.bank"), accountLabel: t("field.account"), nameLabel: t("field.companyName"),
      passportLabel: t("field.passportPinfl"), phoneLabel: t("field.phone"),
    };
  }

  function buildInvoiceHtml(lang) {
    const co = state.company, cl = state.client, pr = state.project, pay = state.payment;
    const currency = pay.currency || "UZS";
    const items = (state.invoiceItems && state.invoiceItems.length) ? state.invoiceItems : [{ desc: pr.name || "IT Services", qty: 1, price: pay.price || 0 }];
    const subtotal = items.reduce((s, it) => s + (Number(it.qty) || 0) * (Number(it.price) || 0), 0);
    const vatRate = pay.vatIncluded ? 0 : (Number(state.meta.defaultVat) || 0);
    const vatAmount = subtotal * (vatRate / 100);
    const total = subtotal + vatAmount;
    const fmt = n => Number(n || 0).toLocaleString("en-US");
    const rowsHtml = items.map((it, i) => `<tr><td>${i + 1}</td><td>${escapeHtml(it.desc)}</td><td>${it.qty}</td><td>${fmt(it.price)} ${currency}</td><td>${fmt((Number(it.qty) || 0) * (Number(it.price) || 0))} ${currency}</td></tr>`).join("");
...
<title>${escapeHtml(t("nav.invoice"))} — ${escapeHtml(state.meta.contractNumber)}</title>
<style>
  body{font-family:system-ui,sans-serif;background:#F4F6FB;margin:0;padding:0 0 60px;color:#111827;}
  .topband{height:8px;background:#1B4DFF;}
  .wrap{max-width:720px;margin:24px auto 0;background:#fff;padding:44px 50px;border-radius:10px;box-shadow:0 10px 40px rgba(17,24,39,.08);}
  .badge{display:inline-block;background:#EAEFFF;color:#1B4DFF;font-weight:700;font-size:12px;letter-spacing:.06em;padding:5px 12px;border-radius:999px;}
  h1{font-size:24px;margin:14px 0 4px;} .muted{color:#5B6472;font-size:13.5px;}
  table{width:100%;border-collapse:collapse;margin:18px 0;font-size:13.5px;}
  th,td{border:1px solid #E3E8F4;padding:8px 10px;text-align:left;} th{background:#F4F6FB;}
  .totals td{border:none;padding:4px 10px;} .totals{width:auto;margin-left:auto;}
  .footnote{text-align:center;color:#5B6472;font-size:11.5px;margin-top:30px;border-top:1px solid #E3E8F4;padding-top:14px;}
</style></head><body>
<div class="topband"></div>
<div class="wrap">
  <span class="badge">${lang === "ru" ? "ЭЛЕКТРОННАЯ КОПИЯ" : lang === "uz" ? "ELEKTRON NUSXA" : "ELECTRONIC COPY"}</span>
  <h1>${escapeHtml(t("nav.invoice"))}</h1>
  <div class="muted">${escapeHtml(t("topbar.contractNo"))}: ${escapeHtml(state.meta.contractNumber)} · ${escapeHtml(state.meta.dateDisplay)}</div>
  <p><strong>${escapeHtml(t("field.companyName"))}:</strong> ${escapeHtml(co.name)} — ${escapeHtml(co.address)}<br>
     <strong>${escapeHtml(t("field.email"))}:</strong> ${escapeHtml(co.email)} · ${escapeHtml(co.phone)}</p>
  <p><strong>${escapeHtml(t("section.client"))}:</strong> ${escapeHtml(cl.name)} — ${escapeHtml(cl.address)}</p>
  <table><thead><tr><th>#</th><th>Description</th><th>Qty</th><th>Unit price</th><th>Amount</th></tr></thead>
  <tbody>${rowsHtml}</tbody></table>
  <table class="totals">
    <tr><td>${escapeHtml(t("field.vat"))} (${vatRate}%)</td><td>${fmt(vatAmount)} ${currency}</td></tr>
    <tr><td><strong>Total</strong></td><td><strong>${fmt(total)} ${currency}</strong></td></tr>
  </table>
  <p class="muted"><strong>${escapeHtml(t("field.bank"))}:</strong> ${escapeHtml(co.bank)} · ${escapeHtml(t("field.account"))}: ${escapeHtml(co.account)} · MFO: ${escapeHtml(co.mfo)}</p>
  <div class="footnote">${lang === "ru" ? "Эта страница — электронная копия счёта, созданного в Uzbekistan IT Contract Generator." : lang === "uz" ? "Ushbu sahifa Uzbekistan IT Contract Generator orqali yaratilgan hisob-fakturaning elektron nusxasidir." : "This page is the electronic copy of an invoice generated by the Uzbekistan IT Contract Generator."}</div>
</div></body></html>`;
  }

  function buildElectronicCopyHtml(docType, lang) {
    if (docType === "invoice") return buildInvoiceHtml(lang);
    const gen = GENERATORS[docType];
    const c = gen.content(state, lang);
    const signLabels = Object.assign({ contractorTitle: c.signContractorTitle, clientTitle: c.signClientTitle }, signLabelsFor());
    let annexRows = null, annexTitle = null;
    if (docType === "itService" && gen.annexRows) {
      annexRows = gen.annexRows(state, lang, t);
      annexTitle = c.annexTitle;
    }
    return WordGen.buildHtmlDoc({
      state, lang, t, docTitle: c.title, docSubtitle: c.subtitle, preamble: c.preamble,
      clauses: c.clauses, signLabels, annexRows, annexTitle,
    });
  }
  ...
  async function generateDocument(docType) {
    ...
    try {
      ...
      const doc = gen.generate(state, lang, t);
      const slug = DOC_SLUGS[docType];
      const filename = `${FILENAME_PREFIX[docType]}-${state.meta.contractNumber}.docx`;
      await WordGen.saveDocument(doc, filename);

      // Also produce the "electronic copy" HTML twin, using the exact filename
      // pattern the cover-page QR code points to (contractNumber-slug.html), so
      // that uploading this file to the URL set in Settings makes the QR work.
      try {
